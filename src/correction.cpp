...
namespace stoat {
    void CorrectionHistoryTable::clear() {
        ...
        std::memset(&m_nonPawnHandTable, 0, sizeof(m_nonPawnHandTable));
    }
    ...
    void CorrectionHistoryTable::update(const Position& pos, i32 depth, Score searchScore, Score staticEval) {
        ...
        m_nonPawnHandTable[pos.stm().idx()][pos.nonPawnHandKey() % kEntries].update(bonus);
    }
    ...
    i32 CorrectionHistoryTable::correction(const Position& pos) const {
        ...
        correction += m_nonPawnHandTable[pos.stm().idx()][pos.nonPawnHandKey() % kEntries];
